...
namespace QaKit.FileComparer.Image
{
	public class ImageDocumentComparer
	{
		...
		public static bool ComparingFilesAreEqual(FileInfo expectedImageFile, FileInfo actualImageFile)
		{
			...
			{
				return ComparingFilesAreEqual(expectedImage, actualImage,
					new FileInfo(actualImageFile.FullName + "_comparingResults.png"));
			}
		}

		public static bool ComparingFilesAreEqual(FileInfo expectedImageFile, FileInfo actualImageFile,
			FileInfo outputDiffFile)
		{
			using (var expectedImage = new MagickImage(expectedImageFile.FullName))
			using (var actualImage = new MagickImage(actualImageFile.FullName))
			{
		...
		public static bool ComparingFilesAreEqual(MagickImage expectedImageFile, MagickImage actualImageFile,
			FileInfo outputDiffFile)
		{
		...
		public static bool AllImageFilesInDirectoryAreEqual(DirectoryInfo expectedImagesFolder,
			DirectoryInfo actualImagesFolder)
		{
			var actualImageFiles = actualImagesFolder.EnumerateFiles("*.*", SearchOption.AllDirectories).Where(file =>
			...
			var expectedImageFiles = expectedImagesFolder.EnumerateFiles("*.*", SearchOption.AllDirectories).Where(
				file => file.Extension.Equals(".bmp", StringComparison.InvariantCultureIgnoreCase) ||
						file.Extension.Equals(".jpg", StringComparison.InvariantCultureIgnoreCase) ||
						file.Extension.Equals(".png", StringComparison.InvariantCultureIgnoreCase) ||
						file.Extension.Equals(".gif", StringComparison.InvariantCultureIgnoreCase));
		...
		public static List<Tuple<FileInfo, FileInfo, FileInfo>> AllImagesInDirectoryAreEqual(
			DirectoryInfo expectedImagesFolder, DirectoryInfo actualImagesFolder)
		{
			var actualImageFiles = actualImagesFolder.EnumerateFiles("*.*", SearchOption.AllDirectories).Where(file =>
				file.Extension.Equals(".bmp", StringComparison.InvariantCultureIgnoreCase) ||
				file.Extension.Equals(".jpg", StringComparison.InvariantCultureIgnoreCase) ||
		...
		/// <param name="expectedImagesFolder"></param>
		/// <param name="actualImagesFolder"></param>
		public static List<ImageCompareResult> AllImagesInDirectoryAreEqual2(DirectoryInfo expectedImagesFolder,
			DirectoryInfo actualImagesFolder)
		{
			var actualImageFiles = actualImagesFolder.EnumerateFiles("*.*", SearchOption.AllDirectories).Where(file =>
				file.Extension.Equals(".bmp", StringComparison.InvariantCultureIgnoreCase) ||
				file.Extension.Equals(".jpg", StringComparison.InvariantCultureIgnoreCase) ||
				file.Extension.Equals(".png", StringComparison.InvariantCultureIgnoreCase) ||
				file.Extension.Equals(".gif", StringComparison.InvariantCultureIgnoreCase)).ToList();

			var expectedImageFiles = expectedImagesFolder.EnumerateFiles("*.*", SearchOption.AllDirectories).Where(file =>
				file.Extension.Equals(".bmp", StringComparison.InvariantCultureIgnoreCase) ||
				file.Extension.Equals(".jpg", StringComparison.InvariantCultureIgnoreCase) ||
				file.Extension.Equals(".png", StringComparison.InvariantCultureIgnoreCase) ||
				file.Extension.Equals(".gif", StringComparison.InvariantCultureIgnoreCase)).ToList();
			...
			List<ImageCompareResult> result = new List<ImageCompareResult>();
			...
			if (actualImageFiles.Count >= expectedImageFiles.Count)
			{
				foreach (FileInfo actualImageFile in actualImageFiles)
				{
					...
					{
						result.Add(new ImageCompareResult
						{
							ActualImage = actualImageFile
						});
						continue;
					}
					var diffFile = new FileInfo(actualImageFile.FullName + "_comparingResults.png");
					var allFilesAreEqual = ComparingFilesAreEqual(expectedImageFile, actualImageFile, diffFile);
					var currentResult = new ImageCompareResult
					{
						ActualImage = actualImageFile,
						ExpectedImage = expectedImageFile,
						DiffImage = !allFilesAreEqual ? diffFile : null
					};
			...
			{
				foreach (FileInfo expectedImageFile in expectedImageFiles)
				{
					var actualImageFile = actualImageFiles.FirstOrDefault(s => s.Name == expectedImageFile.Name);
